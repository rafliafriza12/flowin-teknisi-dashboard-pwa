...
import {
  isAnalyticsEnabled,
  setAnalyticsEnabled,
} from "@/libs/analyticsPreferences";
...
const AnalyticsToggle: React.FC = () => {
  ...
  const handleToggle = () => {
    ...
    showToast.success(
      next ? "Analitik notifikasi diaktifkan" : "Analitik notifikasi dimatikan",
    );
  };
  ...
  return (
    <div className="flex items-center justify-between px-5 py-4">
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          ...
          <p className="text-xs text-grey mt-0.5">
            {mounted
              ? enabled
                ? "Bantu kami meningkatkan pengalaman"
      ...
      <button
        ...
        disabled={!mounted}
        className={`relative w-12 h-6 rounded-full transition-colors duration-200 ${
          enabled ? "bg-[#1F2375]" : "bg-neutral-200"

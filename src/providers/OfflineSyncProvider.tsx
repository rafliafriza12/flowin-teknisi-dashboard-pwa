...
function OfflineBanner({ state }: { state: OfflineSyncState }) {
  ...
  if (pendingCount > 0) {
    return (
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-9999 flex items-center gap-2 bg-amber-600 text-white text-xs font-medium px-4 py-2 rounded-full shadow-xl whitespace-nowrap">
        {isSyncing ? (
          <>
          ...
          <>
            ...
            <button
              type="button"
              onClick={syncNow}
              className="ml-1 underline hover:text-amber-200 transition-colors cursor-pointer"
            >
              Sync
            </button>
          </>
